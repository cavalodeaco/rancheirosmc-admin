...
export function ClassTable({
  ...
  const toggleAll = () =>
    setSelection((current) =>
      current.length === classData.length
        ? []
        : classData.map((item) => item.name)
    );
  ...
  const rows = classData.map((item) => {
    ...
    return (
      <tr key={item.name} className={cx({ [classes.rowSelected]: selected })}>
        ...
        <td>
          <Box className={selectedSearch === "active" ? classes.box : ""}>
            <UnstyledButton onClick={() => handleSearchBy("active")}>
              {item.active === "true" ? (
                <IconCircleCheck />
              ) : (
                <IconCircleMinus />
        ...
        <td>
          <Box className={selectedSearch === "name" ? classes.box : ""}>
            <UnstyledButton onClick={() => handleSearchBy("name")}>
              {item.name}
            </UnstyledButton>
          </Box>
        </td>
        ...
        <td>
          <Box className={selectedSearch === "date" ? classes.box : ""}>
            <UnstyledButton onClick={() => handleSearchBy("date")}>
        ...
        <td>
          <Box className={selectedSearch === "updated_by" ? classes.box : ""}>
          ...
          <Group spacing={0} position="right">
            ...
            >
              ...
              <Menu.Dropdown>
                <Menu.Item
                  disabled={
                    !(admin?.["custom:manager"] || admin?.["custom:download"])
                  }
      ...
      <Table sx={{ minWidth: 800 }} verticalSpacing="sm">
        <thead>
          <tr>
            ...
            <th>
              <UnstyledButton onClick={() => setSearchBy("active")}>
                <Title size={15}>Ativa</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("name")}>
                <Title size={15}>Nome</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("city")}>
                <Title size={15}>Cidade</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("date")}>
                <Title size={15}>Data</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("location")}>
                <Title size={15}>Localização</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("updated_by")}>
                <Title size={15}>Atualizado por</Title>
              </UnstyledButton>
            </th>
            <th>
              <UnstyledButton onClick={() => setSearchBy("updated_at")}>
                <Title size={15}>Data de atualização</Title>
              </UnstyledButton>
            </th>
            <th></th>
          </tr>
