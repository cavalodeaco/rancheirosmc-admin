            ...
            if (selectedClass) {
                ...
                try {
                    ...
                    if (response.status === 200 && message !== "partial") {
                        setAlert({
                            type: "success",
                            title: "Inscrição(ões) chamada(s) para turma!",
                        } as Alert);
                    } else if (
                        response.status === 206 &&
                        message === "partial"
                        ...
                        setAlert({
                            type: "warning",
                            title: "Inscrição(ões) parcialmente chamada(s) para turma!",
                        } as Alert);
                    }
                } catch (error) {
                    setAlert({
        ...
        confirmed: async function () {
            ...
            try {
                ...
                if (response.status === 200 && message !== "partial") {
                    setAlert({
                        type: "success",
                        title: "Inscrição(ões) confirmada(s) para sua(s) turma(s)!",
                    } as Alert);
                } else if (response.status === 206 && message === "partial") {
                    setAlert({
                        type: "warning",
                        title: "Inscrição(ões) parcialmente confirmada(s)!",
                    } as Alert);
                }
            } catch (error) {
                setAlert({
                    type: "error",
                    title: "Falha ao confirmar inscrições!",
                } as Alert);
            }
            setAction(null);
        },
